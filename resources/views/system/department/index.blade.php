...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('system.department.create') }}">Add new department</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($departments as $department)
                <tr role="row" class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('system.department.destroy', ['id' => $department->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('system.department.edit', ['id' => $department->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
